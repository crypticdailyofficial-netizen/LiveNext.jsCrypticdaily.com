/**
 * remove-inline-links.mjs
 *
 * Reverses the changes made by add-inline-links.mjs.
 * For each article in LINK_MAP, removes link markDefs whose hrefs match
 * the injected URLs, strips the mark from affected spans, and merges
 * adjacent spans that have identical marks (to undo the split).
 *
 * Usage:
 *   node remove-inline-links.mjs
 *
 * Optional:
...
// Must match LINK_MAP in add-inline-links.mjs exactly
const LINK_MAP = {
...
// ── Link removal helpers ──────────────────────────────────────────────────────

/**
 * Build the set of full hrefs we want to remove for a given slug.
 */
function targetHrefs(slug) {
  return new Set(
    (LINK_MAP[slug] || []).map((r) => `${BASE_URL}${r.href}`)
  );
}

/**
 * Remove all markDefs whose href is in the target set, strip those mark keys
 * from every span, then merge adjacent spans that have identical mark arrays.
 * Returns { body, removedCount } — body is mutated in place.
 */
function removeLinks(body, hrefs) {
  let removedCount = 0;
  ...
  for (const block of body) {
    ...
    const keysToRemove = new Set(
      (block.markDefs || [])
        .filter((md) => md._type === "link" && hrefs.has(md.href))
        .map((md) => md._key)
    );
    ...
    block.markDefs = (block.markDefs || []).filter(
      (md) => !keysToRemove.has(md._key)
    );

    // Strip mark keys from spans
    for (const child of block.children) {
      if (child._type === "span" && child.marks) {
        child.marks = child.marks.filter((m) => !keysToRemove.has(m));
      }
    }

    // Merge adjacent spans with identical marks
    block.children = mergeSpans(block.children);
  }
...
/**
 * Merge consecutive spans that share the same mark set (order-insensitive).
 */
function mergeSpans(children) {
  const merged = [];
  for (const child of children) {
    if (child._type !== "span" || merged.length === 0) {
      merged.push(child);
      continue;
    }
...
async function main() {
  ...
  console.log(`\n🗑️  Inline link removal — ${slugs.length} article(s)`);
  ...
  for (const slug of slugs) {
    ...
    const removedCount = removeLinks(body, hrefs);
    ...
    console.log(`   🗑️  ${removedCount} link(s) to remove`);

    if (DRY_RUN) {
      console.log(`   — Would patch ${removedCount} removal(s).\n`);
      continue;
    }
    ...
    if (result.error) {
      ...
      console.log(`   ✍️  Patched — ${removedCount} link(s) removed.\n`);
    }
  }

  console.log("─".repeat(50));
  console.log(`🗑️  Links removed: ${totalRemoved}`);
  console.log("");
}
